@php
    ...
    $modulesConfigKey = $modulesConfigKey ?? 'masters.modules';
@endphp

@extends('layouts.admin')

@section('title', $hubLabel . ' — ' . config('app.name'))

@section('breadcrumbs')
    <span class="text-gray-600 font-medium">{{ $hubLabel }}</span>
    <span>/</span>
...
@include('partials.erp.page-header', [
    'title' => $hubLabel . ' Registry',
    'subtitle' => $routePrefix === 'admin.hrm.masters'
        ? 'HRM reference data — organization, shifts, employee setup, biometric devices'
        : 'Reference data for organization, commercial, product, material and sample modules',
])
        ...
        <section class="erp-panel overflow-hidden">
            ...
            <div class="p-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @foreach($modules as $moduleKey)
                    @php $module = config("{$modulesConfigKey}.{$moduleKey}"); @endphp
                    @if($module)
                        <a href="{{ route("{$routePrefix}.index", $moduleKey) }}"
