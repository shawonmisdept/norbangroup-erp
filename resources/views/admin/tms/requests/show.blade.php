...
<p class="flex flex-wrap items-center gap-2"><span class="text-gray-500">Status:</span> @include('partials.erp.tms-status-badge', ['status' => $transportRequest->status, 'label' => $transportRequest->statusLabel()])</p>
...
<p class="flex flex-wrap items-center gap-2"><span class="text-gray-500">Pickup Time:</span> @include('partials.erp.datetime-highlight', ['at' => $transportRequest->pickup_at, 'variant' => 'admin'])</p>
...
@if($transportRequest->trip_log_id)<p class="flex flex-wrap items-center gap-2"><span class="text-gray-500">Trip:</span> <a href="{{ route('admin.tms.trips.show', $transportRequest->trip_log_id) }}" class="erp-btn-sm-secondary">View Trip #{{ $transportRequest->trip_log_id }}</a></p>@endif
@if($transportRequest->rejection_reason)<p class="text-red-600"><span class="text-gray-500">Rejection:</span> {{ $transportRequest->rejection_reason }}</p>@endif
...
<p><span class="text-gray-500">Driver:</span> {{ $transportRequest->driver->displayLabel() }}
...
<p class="flex flex-wrap items-center gap-2">Duty:
...
<p class="mt-2 text-xs text-gray-500">Merged with:
@foreach($transportRequest->tripLog->transportRequests as $r) {{ $r->employee?->name }}@if(!$loop->last), @endif @endforeach
</p>
@endif
</div>
@endif
...
@csrf
<div><label class="erp-label">Reason</label><textarea name="rejection_reason" class="erp-input" rows="3" required></textarea></div>
