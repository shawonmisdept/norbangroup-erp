<div class="space-y-3">
<div><label class="erp-label">Driver Type</label>
...
<div class="company-driver-field"><label class="erp-label">Company Driver</label>
...
<div class="rental-driver-field hidden"><label class="erp-label">Rental Driver</label>
...
<option value="{{ $v->id }}" data-capacity="{{ $v->passenger_capacity }}" @selected(old('vehicle_id') == $v->id)>{{ $v->displayLabel() }} ({{ $v->passenger_capacity }} seats)</option>
@endforeach
</select>
@if(isset($passengerCount))<p class="text-xs text-gray-500 mt-1">Passengers: {{ $passengerCount }}</p>@endif
</div>
</div>
