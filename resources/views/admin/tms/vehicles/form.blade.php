    ...
    <form method="POST" action="{{ $vehicle->exists ? route('admin.tms.vehicles.update', $vehicle) : route('admin.tms.vehicles.store') }}" class="space-y-4" id="vehicle-form">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" class="erp-input" value="{{ old('name', $vehicle->name) }}" required>
            </div>
            <div>
                <label class="erp-label">Registration</label>
                <input type="text" name="reg_number" class="erp-input" value="{{ old('reg_number', $vehicle->reg_number) }}" required>
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div>
                <label class="erp-label">Fuel</label>
            ...
            <div>
                <label class="erp-label">Capacity</label>
                <input type="number" name="passenger_capacity" class="erp-input" value="{{ old('passenger_capacity', $vehicle->passenger_capacity) }}" min="1" required>
            </div>
        </div>
        ...
        <div class="border-t pt-4 rental-fields" id="rental-fields">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="erp-label">Rental Vendor</label>
                ...
                <div>
                    <label class="erp-label">KM Rate Override (BDT/km)</label>
                    <input type="number" step="0.01" min="0" name="rental_km_rate" class="erp-input" value="{{ old('rental_km_rate', $vehicle->rental_km_rate) }}" placeholder="Use vendor/factory default">
                </div>
                <div>
                    <label class="erp-label">Fuel Covered By</label>
                    <select name="fuel_covered_by" class="erp-input">
                        <option value="">—</option>
                ...
                <div>
                    <label class="erp-label">Maintenance Covered By</label>
                    <select name="maintenance_covered_by" class="erp-input">
                        <option value="">—</option>
                        @foreach($paidBy as $k => $l)
        ...
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1">HRM employee — shown on Bill For Posting report</p>
        </div>
