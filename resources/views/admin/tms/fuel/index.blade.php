...
@include('partials.erp.page-header', ['title' => 'Fuel Logs', 'subtitle' => 'Fuel entries per trip', 'actions' => auth()->user()->canManageTmsSubmodule('fuel') ? '<a href="' . route('admin.tms.fuel.create') . '" class="erp-btn-primary !py-2 !px-4 text-xs">Add Entry</a>' : ''])
...
@empty<tr><td colspan="6" class="text-center py-8 text-gray-400">No fuel entries.</td></tr>@endforelse
</tbody></table>
@if($fuelLogs->hasPages())<div class="px-4 py-3 border-t">{{ $fuelLogs->links() }}</div>@endif
</div>
@endsection
