    ...
    <form method="POST" action="{{ route('admin.tms.fuel.store') }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <select name="vehicle_id" class="erp-input" required>
                @foreach($vehicles as $v)
                    <option value="{{ $v->id }}">{{ $v->displayLabel() }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="trip_log_id" class="erp-input">
                <option value="">—</option>
                @foreach($trips as $t)
                    <option value="{{ $t->id }}">#{{ $t->id }} · {{ $t->transportRequest?->employee?->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                <label class="erp-label">Unit</label>
                <input type="text" name="unit" class="erp-input" value="litre">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.001" name="quantity" class="erp-input" required>
            </div>
            <div>
                <label class="erp-label">Unit Price</label>
                <input type="number" step="0.01" name="unit_price" class="erp-input" required>
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="receipt_number" class="erp-input">
        </div>
