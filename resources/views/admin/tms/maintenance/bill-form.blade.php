...
<div><label class="erp-label">Bill / Invoice No</label><input type="text" name="bill_no" class="erp-input @error('bill_no') border-red-400 @enderror" value="{{ old('bill_no', $bill->bill_no) }}" required autofocus>@error('bill_no')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror</div>
<div><label class="erp-label">Bill Date</label><input type="date" name="bill_date" class="erp-input" value="{{ old('bill_date', $bill->bill_date?->format('Y-m-d') ?? now()->toDateString()) }}" required></div>
<div><label class="erp-label">Workshop</label><input type="text" name="workshop_name" class="erp-input" value="{{ old('workshop_name', $bill->workshop_name) }}" required placeholder="JK Motors"></div>
<div><label class="erp-label">Paid By</label><select name="paid_by" class="erp-input">@foreach($paidBy as $k => $l)<option value="{{ $k }}" @selected(old('paid_by', $bill->paid_by) === $k)>{{ $l }}</option>@endforeach</select></div>
</div>
<div class="border-t pt-4">
<div class="flex items-center justify-between mb-2">
<p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Type Of Maintenance</p>
...
<div class="col-span-5"><input type="text" name="items[{{ $i }}][item_name]" class="erp-input" placeholder="Item / service" value="{{ $item['item_name'] ?? '' }}" required></div>
<div class="col-span-2"><input type="number" step="0.001" min="0" name="items[{{ $i }}][quantity]" class="erp-input" placeholder="Qty" value="{{ $item['quantity'] ?? '' }}"></div>
<div class="col-span-2">
<select name="items[{{ $i }}][unit]" class="erp-input">
<option value="">—</option>
@foreach($units as $unit)<option value="{{ $unit }}" @selected(($item['unit'] ?? '') === $unit)>{{ $unit }}</option>@endforeach
</select>
</div>
<div class="col-span-2"><input type="number" step="0.01" min="0" name="items[{{ $i }}][amount]" class="erp-input" placeholder="Amount" value="{{ $item['amount'] ?? '' }}" required></div>
<div class="col-span-1 flex items-center"><button type="button" class="text-xs text-red-600 remove-item-row">×</button></div>
</div>
@endforeach
</div>
</div>
<div><label class="erp-label">Notes</label><textarea name="notes" class="erp-input" rows="2">{{ old('notes', $bill->notes) }}</textarea></div>
<div class="flex gap-2 pt-2"><button type="submit" class="erp-btn-primary">Save Bill</button></div>
</form>
</div>
<script>
