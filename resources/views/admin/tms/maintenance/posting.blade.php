...
@endif
<div><label class="erp-label">Workshop / Vendor</label><input type="text" name="workshop" list="workshop-list" class="erp-input" value="{{ $filters['workshop'] ?? '' }}" required placeholder="JK Motors"><datalist id="workshop-list">@foreach($workshops as $w)<option value="{{ $w }}">@endforeach</datalist></div>
<div><label class="erp-label">From</label><input type="date" name="from" class="erp-input" value="{{ $filters['from'] ?? '' }}" required></div>
<div><label class="erp-label">To</label><input type="date" name="to" class="erp-input" value="{{ $filters['to'] ?? '' }}" required></div>
<div><button type="submit" class="erp-btn-primary w-full">Generate</button></div>
</form>
@if($report)
