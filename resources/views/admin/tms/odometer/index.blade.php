...
<thead><tr><th>Date</th><th>Vehicle</th><th>Driver</th><th>Morning KM</th><th>Evening KM</th><th>Daily KM</th><th>Status</th><th></th></tr></thead>
<tbody>
@forelse($logs as $log)
<tr>
...
<td class="tabular-nums">
    @if($log->hasMorning())
        ...
        @if($log->morningRecordedTime())<p class="text-[10px] text-gray-400 mt-0.5">{{ $log->morningRecordedTime() }}</p>@endif
    @else — @endif
</td>
<td class="tabular-nums">
    @if($log->hasEvening())
        {{ number_format($log->evening_km, 2) }}
        @if($log->eveningRecordedTime())<p class="text-[10px] text-gray-400 mt-0.5">{{ $log->eveningRecordedTime() }}</p>@endif
    @else — @endif
</td>
...
@empty<tr><td colspan="8" class="text-center py-8 text-gray-400">No odometer logs yet.</td></tr>@endforelse
</tbody></table>
