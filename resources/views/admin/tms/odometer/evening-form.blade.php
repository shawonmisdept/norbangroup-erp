...
<div class="erp-panel p-6 max-w-xl">
    <div class="mb-4 p-3 rounded-lg bg-gray-50 border border-gray-100 text-sm">
        ...
        @if($log->morningRecordedTime())
            <p class="text-xs text-gray-400 mt-1">{{ $log->morningRecordedTime() }}</p>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('admin.tms.odometer.evening.store', $log) }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" step="0.01" min="{{ $log->morning_km }}" name="evening_km" class="erp-input" value="{{ old('evening_km') }}" required autofocus placeholder="Min {{ number_format($log->morning_km, 2) }}">
        </div>
