...
<div class="erp-panel p-6 max-w-xl">
    <form method="POST" action="{{ route('admin.tms.odometer.update', $log) }}" class="space-y-4">
        ...
        <div>
            ...
            <select name="factory_id" class="erp-input" required>
                @foreach($factories as $id => $name)
                    <option value="{{ $id }}" @selected(old('factory_id', $log->factory_id) == $id)>{{ $name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="vehicle_id" class="erp-input" required>
                @foreach($vehicles as $v)
                    <option value="{{ $v->id }}" @selected(old('vehicle_id', $log->vehicle_id) == $v->id)>{{ $v->displayLabel() }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="date" name="log_date" class="erp-input" value="{{ old('log_date', $log->log_date?->format('Y-m-d')) }}" required>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                @if($log->morningRecordedTime())
                    <p class="text-[10px] text-gray-400 mt-1">{{ $log->morningRecordedTime() }}</p>
                @endif
            </div>
            <div>
                ...
                @if($log->eveningRecordedTime())
                    <p class="text-[10px] text-gray-400 mt-1">{{ $log->eveningRecordedTime() }}</p>
                @endif
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="notes" class="erp-input" rows="2">{{ old('notes', $log->notes) }}</textarea>
        </div>
