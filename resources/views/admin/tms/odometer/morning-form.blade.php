...
<div class="erp-panel p-6 max-w-xl">
    <form method="POST" action="{{ route('admin.tms.odometer.morning.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <select name="factory_id" class="erp-input" required>
                @foreach($factories as $id => $name)
                    <option value="{{ $id }}" @selected(old('factory_id', $log->factory_id ?? auth()->user()->factory_id) == $id)>{{ $name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="date" name="log_date" class="erp-input" value="{{ old('log_date', $log->log_date?->format('Y-m-d') ?? now()->toDateString()) }}" required>
        </div>

        <div>
            <label class="erp-label">Morning KM</label>
            <input type="number" step="0.01" min="0" name="morning_km" class="erp-input" value="{{ old('morning_km') }}" required autofocus>
        </div>
