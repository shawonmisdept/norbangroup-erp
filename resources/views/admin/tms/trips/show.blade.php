...
<p><span class="text-gray-500">Vehicle:</span> {{ $trip->vehicle?->displayLabel() }} @if($trip->vehicle?->isRental())<span class="erp-badge bg-amber-100 text-amber-800">Rental</span>@endif</p>
@if($trip->vehicle?->rentalVendor)<p><span class="text-gray-500">Vendor:</span> {{ $trip->vehicle->rentalVendor->name }}</p>@endif
...
<p><span class="text-gray-500">KM:</span>
...
@if($trip->total_km !== null) · <strong>{{ number_format($trip->total_km, 2) }} km</strong>@endif
@else
—
@endif
@if($trip->vehicle)<span class="text-xs text-gray-400">(last odometer: {{ number_format($trip->vehicle->last_odometer_km, 2) }})</span>@endif
</p>
<div class="border-t pt-3 mt-3">
<p class="text-gray-500 mb-2 font-medium">Driver Pay</p>
@if($trip->total_driver_pay > 0 || $trip->bill_type !== 'none')
<ul class="space-y-1 text-sm">
@if($trip->night_bill_amount > 0)<li>Night Bill: ৳{{ number_format($trip->night_bill_amount, 2) }}</li>@endif
@if($trip->holiday_duty_amount > 0)<li>Holiday Duty: ৳{{ number_format($trip->holiday_duty_amount, 2) }}</li>@endif
@if($trip->ot_hours > 0)<li>Hourly OT: {{ $trip->ot_hours }} hrs · ৳{{ number_format($trip->ot_hourly_amount, 2) }}</li>@endif
...
<p><span class="text-gray-500">Driver Payment:</span> {{ ucfirst($trip->overtimePayment->payment_status) }}
@if($trip->overtimePayment->payment_status === 'pending' && auth()->user()->hasPermission('tms.overtime.manage'))
<form method="POST" action="{{ route('admin.tms.trips.mark-ot-paid', $trip) }}" class="inline ml-2">@csrf<button type="submit" class="erp-btn-sm-primary" data-confirm="Mark driver payment as paid?">Mark Paid</button></form>
@endif</p>
@endif
</div>
...
@csrf
@if(! $trip->vehicle?->isRental())
<div><label class="erp-label">Start KM</label>
<input type="number" step="0.01" min="0" name="start_km" class="erp-input" placeholder="Min {{ number_format($trip->vehicle?->last_odometer_km ?? 0, 2) }}"></div>
@endif
...
@csrf
@if($trip->start_km !== null)<p class="text-xs text-gray-500">Start KM: {{ number_format($trip->start_km, 2) }}</p>@endif
@if(! $trip->vehicle?->isRental())
<div><label class="erp-label">End KM</label>
<input type="number" step="0.01" min="0" name="end_km" class="erp-input"></div>
@endif
