...
<form method="GET" class="erp-panel p-4 mb-4 flex flex-wrap gap-3 items-end">
    <div>
        ...
        <select name="trip_status" class="erp-input">
            <option value="">All</option>
            @foreach($statuses as $k => $l)
                <option value="{{ $k }}" @selected(($filters['trip_status'] ?? '') === $k)>{{ $l }}</option>
            @endforeach
        </select>
    </div>
        ...
        <tbody>
            @forelse($trips as $trip)
                <tr>
                    ...
                    <td class="text-xs">
                        {{ $trip->transportRequests->pluck('employee.name')->filter()->implode(', ') ?: $trip->transportRequest?->employee?->name }}
                    </td>
            ...
            @empty
                <tr><td colspan="10" class="text-center py-8 text-gray-400">No trips yet.</td></tr>
            @endforelse
        </tbody>
    </table>

    @if($trips->hasPages())
        <div class="px-4 py-3 border-t">{{ $trips->links() }}</div>
    @endif
</div>
@endsection
