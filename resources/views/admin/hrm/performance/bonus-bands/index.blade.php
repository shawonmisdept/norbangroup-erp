...
@section('title', 'Bonus Bands')

@section('breadcrumbs')
    <a href="{{ route('admin.hrm.performance.hub') }}" class="hover:text-brand">Performance</a>
    <span>/</span>
    <span class="text-gray-800 font-medium">Bonus Bands</span>
@endsection

@section('admin-content')
@include('partials.erp.page-header', [
    'title' => 'Performance Bonus Bands',
    'subtitle' => 'Score % → bonus % of gross (factory-wise)',
])

<div class="flex flex-wrap items-center justify-end gap-2 mb-4">
    @if($canManage && $factoryId)
        <a href="{{ route('admin.hrm.performance.bonus-bands.edit', ['factory_id' => $factoryId]) }}" class="erp-btn-primary !py-2 !px-4 text-xs">Edit Bands</a>
    @endif
</div>
                ...
                <tbody>
                    @forelse($bands as $band)
                        <tr>
                            ...
                            <td>{{ number_format($band->bonus_percent, 0) }}%</td>
...
@else
    <div class="erp-panel"><div class="erp-panel-body text-sm text-gray-500">Select a factory to view or edit bonus bands.</div></div>
@endif
@endsection
