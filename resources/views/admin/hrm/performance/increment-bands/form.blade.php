...
@section('title', 'Edit Increment Bands')

@section('breadcrumbs')
    <a href="{{ route('admin.hrm.performance.increment-bands.index', ['factory_id' => $factoryId]) }}" class="hover:text-brand">Increment Bands</a>
    <span>/</span>
    <span class="text-gray-800 font-medium">Edit</span>
@endsection

@section('admin-content')
@include('partials.erp.page-header', ['title' => 'Edit Increment Bands', 'subtitle' => $factoryName])

<form method="POST" action="{{ route('admin.hrm.performance.increment-bands.update') }}" class="max-w-3xl space-y-4">
    @csrf @method('PUT')
            ...
            @foreach(old('bands', $bands) as $i => $band)
                <div class="grid grid-cols-12 gap-2 items-end border-b border-gray-50 pb-3">
                    <div class="col-span-3">
                        @if($i === 0)<label class="erp-form-label">Band</label>@endif
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="number" name="bands[{{ $i }}][min_score]" value="{{ $band['min_score'] ?? 0 }}" class="erp-input !text-xs" step="0.01" required>
                    </div>
                    <div class="col-span-2">
                        @if($i === 0)<label class="erp-form-label">Max %</label>@endif
                        <input type="number" name="bands[{{ $i }}][max_score]" value="{{ $band['max_score'] ?? 100 }}" class="erp-input !text-xs" step="0.01" required>
                    </div>
                    <div class="col-span-2">
                        @if($i === 0)<label class="erp-form-label">Incr. %</label>@endif
                        <input type="number" name="bands[{{ $i }}][increment_percent]" value="{{ $band['increment_percent'] ?? 0 }}" class="erp-input !text-xs" step="0.01" required>
                    </div>
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('admin.hrm.performance.increment-bands.index', ['factory_id' => $factoryId]) }}" class="erp-btn-secondary">Cancel</a>
    </div>
</form>
