...
@include('partials.erp.page-header', ['title' => 'New Gate Pass', 'actions' => '<a href="' . route('admin.hrm.rmg.gate-pass.index') . '" class="erp-btn-secondary">← Back</a>'])
<div class="erp-panel max-w-lg">
    <form method="POST" action="{{ route('admin.hrm.rmg.gate-pass.store') }}" class="erp-panel-body space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            <div><label class="erp-form-label">Out Time</label><input type="time" name="out_time" value="{{ old('out_time') }}" class="erp-input"></div>
            <div><label class="erp-form-label">Expected In</label><input type="time" name="expected_in_time" value="{{ old('expected_in_time') }}" class="erp-input"></div>
        </div>
        <div><label class="erp-form-label">Destination</label><input type="text" name="destination" value="{{ old('destination') }}" class="erp-input"></div>
        <div><label class="erp-form-label">Reason</label><textarea name="reason" rows="2" class="erp-input">{{ old('reason') }}</textarea></div>
        <button type="submit" class="erp-btn-primary">Submit Gate Pass</button>
    </form>
</div>
@endsection
