@auth('rental_driver')
    @php
        $portalUser = auth('rental_driver')->user();
        $unreadCount = $portalUser->unreadNotifications()->count();
        $recentNotifications = $portalUser->notifications()->limit(10)->get();
        $typeMeta = [
            'tms_trip_assigned'  => 'Trip',
            'tms_trip_started'   => 'Trip',
            'tms_trip_completed' => 'Trip',
        ];
    @endphp
    <div class="relative z-50 shrink-0" x-data="notificationBell({{ $unreadCount }}, '{{ route('rental.notifications.unread-count') }}', true)">
        ...
        <div x-show="open" x-cloak x-transition
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
                ...
                @if($unreadCount > 0)
                    <form method="POST" action="{{ route('rental.notifications.read-all') }}">
                        @csrf @method('PATCH')
            ...
            <div class="border-t border-gray-100 px-4 py-2 bg-gray-50">
                <a href="{{ route('rental.notifications.index') }}" class="block text-center text-[11px] font-semibold text-brand hover:text-brand-dark">
