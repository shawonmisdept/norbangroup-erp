...
{
    /** @return list<string> */
    private function performancePermissions(): array
    {
        $perms = array_keys(config('hrm.permissions.performance', []));

        foreach (config('hrm.performance_submodules', []) as $sub) {
    ...
    {
        $extra = $this->performancePermissions();
    ...
    {
        $remove = $this->performancePermissions();
