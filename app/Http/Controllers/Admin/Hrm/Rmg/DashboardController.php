...
use App\Services\Hrm\RmgDashboardService;
...
class DashboardController extends Controller
{
    ...
    public function index(Request $request, RmgDashboardService $dashboard)
    {
        abort_unless($request->user()?->canViewRmgSubmodule('dashboard'), 403);

        $filters = $this->dashboardFilters($request);

        return view('admin.hrm.rmg.dashboard.index', array_merge(
