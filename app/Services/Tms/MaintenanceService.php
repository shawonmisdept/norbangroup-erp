...
class MaintenanceService
{
    ...
    public function normalizeItemsInput(array $items): array
    {
        ...
        foreach ($items as $item) {
            ...
            $normalized[] = [
                'part_catalog_id' => ! empty($item['part_catalog_id']) ? (int) $item['part_catalog_id'] : null,
                'item_name'       => $name,
                'quantity'  => $quantity,
                'unit'      => $unit,
    ...
    public function bulkMarkPostedToFinance(array $billIds, int $userId): int
    {
        ...
        return TmsMaintenanceBill::query()
            ->whereIn('id', $billIds)
            ->whereNull('posted_to_finance_at')
            ->update([
                'posted_to_finance_at' => now(),
                'posted_to_finance_by' => $userId,
                'updated_at'           => now(),
            ]);
    }

    public function unmarkPostedFromFinance(TmsMaintenanceBill $bill): void
    {
        if (! $bill->isPostedToFinance()) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'bill' => 'This bill is not posted to finance.',
            ]);
        }

        $bill->update([
            'posted_to_finance_at' => null,
            'posted_to_finance_by' => null,
        ]);
    }
}
